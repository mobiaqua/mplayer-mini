...
static int select_subtitle(MPContext *mpctx)
{
    ...
    if (!found && sub_id >= 0) {
        // if user asks for a sub id, use that next.
        id    = sub_id;
    ...
    if (!found && sub_id == -1) {
        ...
        if (sub_id == -1 && sub_lang)
            sub_id = demuxer_sub_track_by_lang(mpctx->demuxer, sub_lang);
        if (sub_id == -1)
            sub_id = demuxer_default_sub_track(mpctx->demuxer);
        if (sub_id >= 0) {
            id    = sub_id;
            found = mp_property_do("sub_demux", M_PROPERTY_SET, &id, mpctx) == M_PROPERTY_OK;
        }
...
//============ Open DEMUXERS --- DETECT file type =======================
    ...
    mpctx->demuxer = demux_open(mpctx->stream, mpctx->file_format, audio_id, video_id, sub_id, filename);
